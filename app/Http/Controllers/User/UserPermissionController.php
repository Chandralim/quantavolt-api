...
class UserPermissionController extends Controller
{
  ...
  public function getActionPermissions(){
    return response()->json([
      "data"=>\App\Model\ActionPermission::get(),
    ],200);    
  }

  public function getDataPermissions(){
    return response()->json([
      "data"=>\App\Model\DataPermission::get(),
    ],200);    
  }

  public function show(UserPermissionRequest $request)
  {
    MyLib::checkScope($this->auth,['ap-user_permission-view']);
  ...
  {
    MyLib::checkScope($this->auth,['ap-user_permission-edit']);
    ...
    $data_permissions_in = json_decode($request->data_permissions, true);
        ...
        $rules = [
            ...
            'action_permissions.*.id' => 'required|exists:\App\Model\ActionPermission,id',
            'data_permissions' => 'nullable|array',
            'data_permissions.*.id' => 'required|exists:\App\Model\DataPermission,id',
        ];
    ...
    try {
      ...
      \App\Model\UserPermission::where("user_id",$model_query->id)->delete();
      ...
      foreach ($action_permissions_in as $key => $value) {
          ...
          $child_query = new \App\Model\UserPermission();
